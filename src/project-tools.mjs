...
export const projectTools = [
  { name: 'astra_project_search', description: 'Search project text with bounded output. Literal by default; regex requires ripgrep. Skips hidden, private and generated directories, environment files, binary files and symlinks. glob supports *, ** and ?. Results explicitly report partial searches.',
  ...
  { name: 'astra_project_read', description: 'Read a bounded UTF-8 line range and obtain the complete file SHA-256 needed for an edit. Files must be at most 2 MiB; symlink paths are refused. The hash covers original bytes, including line endings.',
  ...
  { name: 'astra_git_status', description: 'Read bounded Git status and optional staged/unstaged diffs without staging or committing. Disables external diff, text conversion, filesystem monitors, pagers and optional index writes. Exits, truncation and timeouts are reported.',
...
function projectEdit(args, guard, signal) {
  ...
  const temporary = path.join(path.dirname(target), `.astra-edit-${crypto.randomUUID()}.tmp`);
  let fd;
  try {
    ...
    // This prevents competing Astra calls interleaving; external writers cannot
...
export async function callProjectTool(name, input, options = {}) {
  try {
    ...
    const guard = createPathGuard({ home: options.home || os.homedir(), roots: options.roots ?? options.env?.ASTRA_FILE_ROOTS ?? process.env.ASTRA_FILE_ROOTS });
    let result;
    if (name === 'astra_project_read') result = projectRead(args, guard);
    else if (name === 'astra_project_edit') result = projectEdit(args, guard, options.signal);
    else if (name === 'astra_project_search') result = await projectSearch(args, guard, options);
