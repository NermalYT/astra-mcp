...
export function assessPlatform({ platform, arch = process.arch, release = '', osRelease = {}, windowsEdition = '', windowsBuild } = {}) {
  ...
  if (!['x64', 'arm64'].includes(arch)) return { ...base, reason: 'Astra requires x86-64 or ARM64.' };
  if (platform === 'linux') {
    ...
    if (!(ubuntu || derived) || !Object.hasOwn(UBUNTU_LTS, codename || '')) {
      return { ...base, reason: 'Astra supports Ubuntu 22.04, 24.04 or 26.04 LTS and derivatives declaring the same Ubuntu base in /etc/os-release.' };
    }
  ...
  return { ...base, reason: 'Astra supports only Windows 11 Pro through WSL2 and Ubuntu LTS-based Linux.' };
}
...
export async function assertSupportedPlatform({ allowWindowsLauncher = false } = {}) {
  ...
  if (result.mode === 'windows-wsl2-launcher' && !allowWindowsLauncher) {
    throw new Error('Run scripts/install.ps1 from Windows PowerShell. Astra itself runs inside WSL2 Ubuntu LTS; a separate native Windows cursor is not available.');
  }
  return result;
}
