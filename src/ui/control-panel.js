...
if (incomingToken) sessionStorage.setItem('astra-panel-token', incomingToken);
const token = incomingToken || sessionStorage.getItem('astra-panel-token');
...
async function api(route, body) {
  ...
  if (!result.ok) throw new Error(data.error || 'Astra request failed.');
  return data;
}
...
async function refresh() {
  ...
  try {
    ...
    $('session-title').textContent = 'Astra ' + (data.version || '');
...
if (!token) reportError('Open the current private control link returned by astra_control_panel.');
