...
export const extraTools = [
  {
    ...
    description: 'Read loaded text and links from Astra browser tabs without selecting or foregrounding them. Works in Astra’s hidden browser while the user works elsewhere. Reads beyond the viewport; unloaded lazy content still requires scrolling. Current, all (first 16), or zero-based tab indices. Reports errors and omitted indices individually. Does not access tabs in a separate personal browser.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'browser_scrape',
    description: 'Read 1–8 HTTP(S) URLs in temporary Astra background tabs, up to 3 at once. Returns compact page text, title, links, HTTP status and individual failures. Uses Astra browser cookies. Closes only its temporary tabs, preserves existing tabs, and never moves the user’s OS cursor. Waits for DOMContentLoaded; pages requiring later rendering or scrolling may need normal browser tools.',
    inputSchema: {
      type: 'object',
      properties: {
...
function buildCode(name, opts) {
  ...
  if (name === 'browser_read_tabs') return `async (page) => {
    ...
    return { astra_browser_result: 1, tool: 'browser_read_tabs', total_tabs: tabs.length, omitted_indices: opts.tabs === 'all' ? tabs.map((_, i) => i).slice(16) : [], pages };
  }`;
  return `async (page) => {
    ${prelude}
    ...
    return { astra_browser_result: 1, tool: 'browser_scrape', pages };
  }`;
}
...
function unpack(result, name) {
  ...
  for (let candidate of candidates) {
    try {
      ...
      if (candidate?.astra_browser_result !== 1 || candidate.tool !== name || !Array.isArray(candidate.pages)) continue;
      const { astra_browser_result, ...payload } = candidate;
  ...
  return fail(`Astra could not decode ${name}'s browser result. The browser operation may have run; check existing tabs before retrying. ${result?.content?.filter(c => c.type === 'text').map(c => c.text).join('\n').slice(0, 1500) || 'No text returned.'}`);
}

export async function callExtraTool(name, args = {}, invoke, signal) {
  if (!extraTools.some(tool => tool.name === name)) return fail(`Unknown Astra browser helper: ${name}.`);
  try {
