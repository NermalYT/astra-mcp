export const discoveryTools = [
  { name: 'astra_tools', description: 'Search all Astra capabilities and retrieve exact input schemas before calling them. Empty query lists tool names. Works in compact and full modes.',
  ...
  { name: 'astra_call', description: 'Call one Astra tool by its exact name and arguments from astra_tools. Applies the original schema, cancellation and serialization. Does not retry. Read image results if your model supports vision.',
...
export function advertisedTools(tools, mode) {
  ...
  const initial = new Set(['astra_status', 'astra_tools', 'astra_call', 'astra_control_panel', 'astra_help']);
