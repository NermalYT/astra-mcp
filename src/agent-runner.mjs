...
export const agentTools = [
  { name: 'astra_agent_start', description: 'Explicitly start a bounded background agent using a named model on the local LM Studio server and only the listed tools. No model download, cloud service, API key or automatic retry. Agents share Astra\'s desktop and files: use separate project copies for concurrent edits. Tool permissions are not a security sandbox. Inspect status before starting another attempt.',
  ...
  { name: 'astra_agent_status', description: 'List recent local agents or read one bounded progress log and final report. Runs and reports are held in memory, not resumed after restart. Cancellation prevents further dispatch; an in-flight tool may already have changed state.',
  ...
  { name: 'astra_agent_cancel', description: 'Cancel a local model loop and abort its current request/tool signal. Does not undo completed actions or guarantee that an already dispatched external action stopped. Check status before further work.',
...
const forbidden = name => /^astra_(?:agent(?:_|$)|control(?:_|$)|schedule(?:_|$))/.test(name) || ['astra_call', 'astra_tools'].includes(name);
...
export function createAgentRunner({ invoke, listTools, env = process.env, baseURL = 'http://127.0.0.1:1234/v1', maxConcurrent = 2 } = {}) {
  ...
  const get = id => { const run = runs.get(id); if (!run) throw new Error('Unknown or expired agent ID; list agents with astra_agent_status.'); return run; };
  ...
  return {
    async start(input, signal) {
      ...
      checkAbort(signal);
      const args = validate('astra_agent_start', input);
    ...
    status(input = {}) {
      const args = validate('astra_agent_status', input);
      if (!args.agent_id) {
    ...
    cancel(id) { validate('astra_agent_cancel', { agent_id: id }); const run = get(id); stop(run, 'cancelled by request'); return snapshot(run); },
    async close() {
      if (closed) return;
      closed = true;
...
export async function callAgentTool(name, input, runner, signal) {
  ...
  const result = name === 'astra_agent_start' ? await runner.start(args, signal)
    : name === 'astra_agent_status' ? runner.status(args) : runner.cancel(args.agent_id);
