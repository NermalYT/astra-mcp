...
export const desktopTools = [
  { name: 'astra_desktop_state', description: 'Observe Astra’s private desktop: windows, active window, screen/cursor metadata and optional screenshot in one call. Use before selecting coordinates. screenshot=false supports text-only models.',
    inputSchema: { type: 'object', properties: { screenshot: { type: 'boolean', default: true } }, additionalProperties: false },
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false } },
  { name: 'astra_desktop_actions', description: 'Run 1–12 already-planned private-desktop actions, then optionally screenshot. Uses existing tool argument schemas. Validates every argument schema first, holds desktop focus through the sequence, stops on the first failure and never replays actions. Observe state first; do not guess coordinates. screenshot=false for text-only models.',
...
export async function callDesktopTool(name, args, invokeBatch, signal, options) {
  const steps = name === 'astra_desktop_state'
