...
export async function startRuntime(base, settings = {}) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-display-'));
  ...
  function launch(command, args, options = {}) {
    ...
    child.stderr?.on('data', b => { if (settings.debug) process.stderr.write(`[astra/runtime] ${b}`); });
    children.push(child);
    return child;
  }
  const close = async () => {
  ...
  try {
    ...
    const binary = process.env.ASTRA_XVFB || settings.desktop?.xvfbPath || (await fs.access(path.join(dataDir, 'runtime/usr/bin/Xvfb')).then(() => path.join(dataDir, 'runtime/usr/bin/Xvfb')).catch(() => 'Xvfb'));
    ...
    Object.assign(env, { ASTRA_APP_HOME: appHome, ASTRA_HOST_DATA_HOME: process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local/share'), XDG_RUNTIME_DIR: privateRuntime,
