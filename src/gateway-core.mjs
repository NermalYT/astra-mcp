...
export class SerialQueue {
  ...
  run(fn, signal) {
    ...
    if (this.jobs.length >= this.limit) return Promise.reject(new Error('Astra queue is full; no action was sent.'));
  ...
  cancel() {
    for (const job of this.jobs.splice(0)) {
      job.signal?.removeEventListener('abort', job.abort);
      job.reject(new Error('Astra is closing; no action was sent.'));
    }
  }
}
...
export class Gateway {
  constructor(config, options = {}) {
    if (!config || Array.isArray(config) || typeof config !== 'object') throw new Error('Astra backend configuration must be an object.');
    this.options = options;
    this.settings = options.settings || {};
    this.log = options.log || (line => console.error(`[astra] ${line}`));
    ...
    this.queues = new Map();
    this.reserved = new Set(['astra_status', ...(options.reservedNames || [])]);
  ...
  async ensureConnected(peer) {
    if (this.closing) throw new Error('Astra is closing.');
  ...
  async connect(peer) {
    ...
    const client = new Client({ name: `astra-${peer.name}`, version: VERSION });
    ...
    if (useRuntime && runtimeEnv) {
      for (const key of ['DISPLAY', 'XAUTHORITY', 'WAYLAND_DISPLAY', 'XDG_SESSION_TYPE', 'SESSION_MANAGER', 'GDK_BACKEND', 'QT_QPA_PLATFORM', 'DBUS_SESSION_BUS_ADDRESS', 'DBUS_STARTER_ADDRESS', 'DBUS_STARTER_BUS_TYPE', ...(peer.entry.isolated ? ['HOME', 'XDG_CONFIG_HOME', 'XDG_DATA_HOME', 'XDG_CACHE_HOME', 'XDG_RUNTIME_DIR', 'ASTRA_APP_HOME'] : [])]) {
  ...
  async invoke(name, args = {}, signal, options = {}) {
    ...
    if (!route) return toolError(`Unknown tool: ${name}. Refresh the Astra tool list.`);
  ...
  async invokeDesktopBatch(steps, signal, options = {}) {
    ...
    // changed by another Astra request in the middle of a sequence.
    try {
      aborted(signal);
  ...
  async dispatch(name, route, args, signal, options = {}) {
    ...
    if ((peer.group === 'desktop' || peer.name === 'terminal-files') && this.options.runtimeStatus?.().healthy === false) {
      return toolError('Astra isolated desktop is unavailable. Restart the Astra integration; no action was sent to any desktop.');
    }
    try { await this.ensureConnected(peer); } catch (error) { return toolError(`${peer.name} is unavailable: ${errorText(error)}. No action was sent; check astra_status.`); }
    aborted(signal);
    ...
    try {
      ...
      await peer.closePromise;
      return toolError(`${peer.name}/${route.original}: ${errorText(error)}. ${dispatched ? 'The action may have run; check actual state before retrying. Astra did not replay it.' : 'No action was sent.'}`);
    } finally {
