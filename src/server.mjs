...
const server = new Server({ name: 'astra', version: '2.0.0' }, {
  capabilities: { tools: { listChanged: true } },
  instructions: 'Astra provides a private AI desktop with its own cursor and keyboard, plus a headless browser. The human can work simultaneously without AI mouse/keyboard interference. Browser tools inspect background tabs through browser APIs; screen visibility is irrelevant. Prefer browser tools for websites and scrape tasks; use astra browser helpers when available to reduce round trips. Native desktop tools see only Astra\'s isolated desktop, not the human\'s desktop. Launch native apps inside Astra before interacting. Observe, act, verify. Return actual errors honestly and use short valid JSON. Never automatically retry a dispatched action after a timeout or error; check actual state first. This isolation is a workspace convenience, not a filesystem/security sandbox.'
});
...
try {
  ...
  connected = true;
  console.error(`[astra] Ready: ${gateway.routes.size} backend tools + ${extraTools.length + 1} Astra tools`);
} catch (error) {
