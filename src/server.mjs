...
const server = new Server({ name: 'astra', version: VERSION }, {
  ...
  instructions: 'Astra provides a private AI desktop with its own cursor and keyboard, plus a headless browser. The human can work simultaneously without AI input interference. Use browser tools for websites; browser_read_tabs/browser_scrape reduce round trips. Use astra_desktop_state to observe native apps and astra_desktop_actions for a short sequence of already-grounded actions followed by verification. Set screenshot=false for text-only models. Prefer direct project file edits and application scripting over repeated GUI typing. Launch GUI apps with application_launch executable/args/cwd or desktop_id inside Astra; use astra_job_start for long command-line builds, renders and tests, then inspect status without blocking other work. Jobs and GUI apps use private HOME profiles; project cwd stays accessible. Use CLI batch/headless modes for Blender/Unity when appropriate, and verify files/exit codes. The private Xvfb desktop does not guarantee GPU-accelerated studio viewports. Native desktop tools see only Astra\'s desktop. Observe, act, verify; obey application/project permissions. Return actual errors and valid JSON. Never automatically retry a dispatched action or job after a timeout/error; check its state first. Input isolation is not a filesystem/security sandbox.'
});
...
try {
  ...
  runtime = process.env.ASTRA_SKIP_RUNTIME === '1'
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: [
    ...
    { name: 'astra_status', description: 'Check desktop health, connections, timing and errors. reconnect=true reconnects unavailable backends without replaying actions.', inputSchema: { type: 'object', properties: { reconnect: { type: 'boolean', default: false } }, additionalProperties: false }, annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false } }
  ] }));
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    const { name, arguments: args } = request.params;
    if (name === 'astra_status') {
      if (args && (Object.keys(args).some(k => k !== 'reconnect') || (args.reconnect !== undefined && typeof args.reconnect !== 'boolean'))) return toolError('astra_status accepts only reconnect: boolean.');
  ...
  connected = true;
  console.error(`[astra] Ready: ${gateway.routes.size} backend tools + ${helperTools.length + 1} Astra tools`);
} catch (error) {
