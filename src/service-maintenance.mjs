...
const rootFiles = new Set(['package.json', 'package-lock.json', 'LICENSE', '.gitignore', '.gitattributes', 'INSTALL_ASTRA.sh', 'INSTALL_ASTRA.cmd', 'install-astra.py', ...manifests]);
...
function decodeManifest(bytes) {
  ...
  if (manifest?.format !== 1 || manifest?.project !== 'astra-mcp' || !semver.test(manifest.version) || !Array.isArray(manifest.files) || manifest.files.length > MAX_FILES) throw new Error('Invalid installed source manifest.');
  const files = new Map();
...
spec = importlib.util.spec_from_file_location('astra_trusted_release_verifier', sys.argv[1])
...
export function createServiceMaintenance({ root, dataDir = path.join(os.homedir(), '.local/share/astra-mcp'), fetchImpl = globalThis.fetch, pythonCommand = process.platform === 'win32' ? 'python' : 'python3' } = {}) {
  ...
  function status(signal) {
    ...
    return { installation_root: root, data_directory: dataDir, version: pkg.version || null, source_revision: current.revision, source_files: current.files.size, source_bytes: current.bytes, baseline: base ? { path: path.join(root, base.name), version: base.manifest.version } : null, modifications: modifications(current.files, base), upgrade_guide: path.join(root, 'docs', 'UPGRADING.md'), official_guide: 'https://github.com/NermalYT/astra-mcp/blob/main/docs/UPGRADING.md', backups: list('backup'), stages: list('stage'), maintenance_lock: fs.existsSync(path.join(storage, 'writer.lock')) ? path.join(storage, 'writer.lock') : null, activation: 'Restart Astra through the MCP host after reporting results; this tool never kills the serving process.' };
  }
  ...
  async function download(url, limit, signal) {
    ...
    for (let redirects = 0; redirects <= 4; redirects++) {
      ...
      const response = await fetchImpl(address.href, { signal: combined, redirect: 'manual', headers: { 'User-Agent': 'Astra-local-service-maintenance', Accept: 'application/octet-stream' } });
  ...
  async function prepare(version, signal) {
    ...
    const filename = `Astra-${version}-${targetPlatform === 'windows' ? 'Windows-11-Pro-WSL2.zip' : 'Linux-Ubuntu-LTS.tar.gz'}`;
    const releaseURL = `https://github.com/NermalYT/astra-mcp/releases/download/v${version}/`;
  ...
  async function run(input, signal) {
    ...
    return locked(async () => {
      ...
      if (input.action === 'restore') {
        ...
        return { restored: input.backup_id, dependency_review_required: dependencyReview, ...(dependencyReview ? { activation: 'Source was restored but installed dependencies may differ. Stop Astra, review package/lock changes and run the appropriate dependency installation in a recovery terminal before restarting. No dependency command was run.' } : {}), ...transact(current, saved.files, signal, `Before restoring ${input.backup_id}`) };
      }
      ...
      if (dependencyShape(current.files) !== dependencyShape(saved.files)) throw new Error(`Prepared release changes dependencies, lockfile resolution or runtime requirements. Automatic apply refused. Review ${path.join(storage, input.stage_id, 'source')} and install separately with Astra stopped.`);
      ...
      const manifest = { format: 1, project: 'astra-mcp', version: saved.record.version, platform: release.manifest.platform, files: [...saved.files].filter(([name]) => !manifests.has(name)).map(([name, file]) => entry(name, file)) };
...
export async function callServiceTool(name, input, manager, signal) {
  if (name !== 'astra_service') return undefined;
