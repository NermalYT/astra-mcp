...
export function createControlState({ onStop = async () => {} } = {}) {
  ...
  return {
    ...
    resume() { if (closed) throw new Error('Astra session is closing.'); paused = false; return snapshot(); },
    async stop() {
      paused = true;
      for (const entry of active.values()) entry.controller.abort(new Error('Stopped from Astra control panel; inspect state before repeating work.'));
      await onStop();
      return snapshot();
    },
    async close() {
      ...
      for (const entry of entries) entry.controller.abort(new Error('Astra session is closing.'));
    ...
    async run(name, fn, signal) {
      if (closed) throw new Error('Astra session is closing.');
      if (paused) throw new Error('Astra is paused from the user control panel. No new action was sent.');
