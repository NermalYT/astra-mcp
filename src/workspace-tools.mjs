...
export function createWorkspaceManager({ dataDir, env = process.env, home = os.homedir(), runtimeStatus = () => ({ healthy: true }), runVersion = exec, lockWaitMs = 2000 } = {}) {
  if (typeof dataDir !== 'string' || !path.isAbsolute(dataDir)) throw new Error('An absolute Astra dataDir is required.');
  if (!Number.isInteger(lockWaitMs) || lockWaitMs < 100 || lockWaitMs > 5000) throw new Error('Invalid workspace lock wait.');
  const guard = createPathGuard({ home, roots: env.ASTRA_FILE_ROOTS });
  const privateEnv = { ...env, ...(env.ASTRA_APP_HOME ? { HOME: env.ASTRA_APP_HOME } : {}) };
  let rootPromise;
  ...
  async function probe(input = {}, signal) {
    ...
    const inspect = async (name, explicit) => {
      ...
      try {
        checkSignal(signal);
        const pending = runVersion(found.path, applications[name].args, { env: privateEnv, cwd: env.ASTRA_APP_HOME || home, timeout: timeout_ms, killSignal: 'SIGKILL', signal, maxBuffer: 4096, windowsHide: true });
...
export async function callWorkspaceTool(name, input, manager, signal) {
  checkSignal(signal);
  const result = name === 'astra_workspace_notes' ? await manager.notes(input, signal) : name === 'astra_app_probe' ? await manager.probe(input, signal) : (() => { throw new Error(`Unknown workspace tool: ${name}`); })();
