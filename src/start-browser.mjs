// A persistent, private browser profile per active Astra connection.
...
const root = process.env.ASTRA_BROWSER_PROFILE || path.join(dataDir, 'browser-profiles');
...
if (!profile) throw new Error('All Astra browser profile slots are busy');
...
child.on('error', e => { console.error('[astra/browser]', e.message); stop(1); });
