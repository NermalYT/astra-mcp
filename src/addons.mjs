...
const idOf = id => { if (typeof id !== 'string' || !ID.test(id) || id === 'astra' || id.startsWith('astra_')) throw new Error('Addon id must use 1–32 lowercase letters, digits or underscores, start with a letter, and not use the astra namespace.'); return id; };
...
export function validateAddonManifest(value) {
  keys(value, ['id', 'name', 'version', 'description', 'astra_compat', 'platforms', 'server'], 'addon manifest');
  ...
  if (value.astra_compat !== COMPAT) throw new Error(`Addon astra_compat must be exactly "${COMPAT}".`);
...
async function manifestAt(directoryPath) {
  const { bytes } = await regularFile(path.join(directoryPath, 'astra-addon.json'), 64 * 1024);
  let parsed;
  try { parsed = JSON.parse(bytes.toString('utf8')); } catch { throw new Error('Invalid astra-addon.json JSON.'); }
...
export function createAddonManager({ dataDir, platform = process.env.WSL_DISTRO_NAME || process.env.WSL_INTEROP ? 'windows-wsl2' : 'linux' } = {}) {
  ...
  const install = async (source, { trust = false, signal } = {}) => {
    ...
    if (!relativeStorage || (!relativeStorage.startsWith(`..${path.sep}`) && relativeStorage !== '..' && !path.isAbsolute(relativeStorage))) throw new Error('Addon source must not contain Astra addon storage.');
...
export async function callAddonTool(name, input, manager, signal) {
  if (name !== 'astra_addons') throw new Error(`Unknown addon tool: ${name}`);
  check(signal);
