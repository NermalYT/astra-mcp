...
export function createPathGuard({home=os.homedir(),roots=process.env.ASTRA_FILE_ROOTS}={}) {
  const configured=typeof roots==='string'?JSON.parse(roots):roots;
  if(configured!==undefined&&(!Array.isArray(configured)||configured.length>16||configured.some(r=>typeof r!=='string'||!path.isAbsolute(r))))throw new Error('ASTRA_FILE_ROOTS must be a JSON array of at most 16 absolute directories.');
