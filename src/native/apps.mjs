...
server.registerTool(
    "application_launch",
    {
        description: "Launch a GUI in Astra's private desktop. Use desktop_id for an installed app, or executable with args for Blender, Unity, an IDE, or a project file. cwd selects the project directory. wait_ms is a maximum readiness wait; returns early when a new window appears. A window does not guarantee an app has finished loading its project.",
    ...
    async ({ desktop_id, executable, args, cwd, wait_ms }) => {
        ...
        const observed = await launchAndObserve(command, launchArgs, {
            ...
            env: { ...process.env, HOME: process.env.ASTRA_APP_HOME || process.env.HOME },
            windows: getWindows,
            timeoutMs: wait_ms,
        });
        return jsonResult({
...
async function main()
{
    ...
    console.error(
        "[local-astra-desktop-apps] MCP server started."
    );
...
main().catch(
    error => {
        ...
        console.error(
            "[local-astra-desktop-apps] Fatal error:",
            error,
        );
