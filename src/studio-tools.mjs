...
export const studioTools = [
  {
    name: 'astra_job_start',
    description: 'Start a background build, test, render or other command using explicit executable and argv. Returns a job ID immediately; inspect astra_job_status later. Private AI display/home; no shell parsing. Commands can modify project files. Never retry a dispatched job without checking its status.',
    inputSchema: { ...plain({
  ...
  {
    name: 'astra_job_cancel', description: 'Request cancellation of a background job and its process group. Inspect status to confirm cleanup. Already completed jobs are returned unchanged.',
...
export function createStudioManager({ env = process.env, home = os.homedir(), roots = env.ASTRA_FILE_ROOTS,
  ...
  const jobs = new Map();
  const jobEnv = { ...env, ...(env.ASTRA_APP_HOME ? { HOME: env.ASTRA_APP_HOME } : {}) };
  let closed = false;
  ...
  const get = id => { const job = jobs.get(id); if (!job) throw new Error('Unknown or expired job ID; list jobs with astra_job_status.'); return job; };
  ...
  return {
    async start(input, signal) {
      ...
      if (process.platform !== 'linux') throw new Error('Studio jobs run on Linux; Windows 11 Pro uses Astra inside WSL2.');
      ...
      await readyPromise;
      return { ...snapshot(job), private_home: Boolean(env.ASTRA_APP_HOME), next_cursor: 0 };
    },
    status(input = {}) {
...
export async function callStudioTool(name, input, manager, signal) {
  let result;
  if (name === 'astra_job_start') result = await manager.start(input, signal);
  else if (name === 'astra_job_status') result = manager.status(input);
  else if (name === 'astra_job_cancel') result = manager.cancel(input.job_id);
