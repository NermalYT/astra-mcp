...
Astra connects a tool-capable model to a background browser, an independent Linux
desktop, application/window controls, screenshots, mouse, keyboard, terminal,
and files. Version **2.1.0 exposes 84 tools through one `mcp/astra` switch** in LM Studio:
all 79 previous tools, two desktop workflow helpers, and three studio job tools.
The model runs in your chosen host; Astra does not bundle or lock you to a model.

```mermaid
flowchart LR
    M[Compatible model in LM Studio] --> A[Astra MCP]
...
- **Combine desktop work.** `astra_desktop_state` returns window/input state and an
  optional screenshot. `astra_desktop_actions` validates and runs up to 12 actions
...
- **Run longer tasks.** `astra_job_start`, `astra_job_status`, and `astra_job_cancel`
...
| Host | How Astra runs | Validation status |
| --- | --- | --- |
...
| Windows 11 Pro / Pro N, x64 or ARM64 | Windows LM Studio launches Astra inside **WSL2 Ubuntu LTS** | Installer and platform checks provided; no Windows 11 end-to-end hardware run yet |
...
| --- | --- | --- |
| Ubuntu LTS / Linux Mint XFCE | [Astra 2.1.0 for Linux](https://github.com/NermalYT/astra-mcp/releases/download/v2.1.0/Astra-2.1.0-Linux-Ubuntu-LTS.tar.gz) | Read `START_HERE.md`; run `bash INSTALL_ASTRA.sh` in a terminal |
| Windows 11 Pro / Pro N | [Astra 2.1.0 for Windows through WSL2](https://github.com/NermalYT/astra-mcp/releases/download/v2.1.0/Astra-2.1.0-Windows-11-Pro-WSL2.zip) | Read `START_HERE.md`; run `INSTALL_ASTRA.cmd` |

The archives contain the same Astra source and tools with different starter
documents and launchers. Compare downloads with the release's
[SHA256SUMS](https://github.com/NermalYT/astra-mcp/releases/download/v2.1.0/SHA256SUMS).
...
require a Windows restart before running Astra setup. Follow the
...
original Astra registrations while retaining unrelated integrations. Then restart
LM Studio, load a compatible model, and enable **mcp/astra** in the chat.
...
`--replace-legacy` removes the six earlier Astra registrations; unrelated servers
...
models have not been benchmarked or certified against Astra.**
...
they do not enforce memory or GPU limits and stop when Astra shuts down.

Blender **4.5.13 LTS** was exercised on Linux Mint 22.3: a small 64×64 Cycles **CPU**
render produced PNG and `.blend` files; the GUI opened the scene on Astra's private
...
Astra reads **its own** browser tabs and private desktop. It does not automatically
...
restarting Astra; those applications close with the session.
...
Private data defaults to `~/.local/share/astra-mcp`, outside the repository.
`ASTRA_DATA_DIR` selects another directory. Optional `settings.json` in that folder:
...
Use `astra_status` for connections, queues, call timing and private-display health.
