...
| --- | --- | --- |
| Ubuntu LTS / Linux Mint XFCE | [Astra 3.0.0 for Linux](https://github.com/NermalYT/astra-mcp/releases/download/v3.0.0/Astra-3.0.0-Linux-Ubuntu-LTS.tar.gz) | Read `START_HERE.md`; open `INSTALL_ASTRA.sh` for graphical setup |
| Windows 11 Pro / Pro N | [Astra 3.0.0 for Windows through WSL2](https://github.com/NermalYT/astra-mcp/releases/download/v3.0.0/Astra-3.0.0-Windows-11-Pro-WSL2.zip) | Read `START_HERE.md`; run `INSTALL_ASTRA.cmd` |

Both packages contain the same source and tools with different launchers and starter instructions. Compare the archive with the release's [SHA256SUMS](https://github.com/NermalYT/astra-mcp/releases/download/v3.0.0/SHA256SUMS). Each package includes a file manifest. Dependencies and models are downloaded separately; first setup needs internet access.
...
| Entry tool | Purpose |
| --- | --- |
| `astra_tools` | Search capabilities and retrieve exact argument schemas |
| `astra_call` | Invoke a discovered tool through its original validation and execution path |
| `astra_status` | Check available tools, runtime health, activity and connections |
| `astra_control_panel` | Return your private local control-panel link |
| `astra_help` | Read workflow, browser, desktop, studio or compatibility guidance |
...
Keep the session link private. With Node/npm available, run `npm run control` from an Astra source directory in Linux or WSL to print the current link while Astra is running. No model API is needed to use the panel. [Platform and networking details](docs/PLATFORMS.md#compact-mode-and-the-control-panel).
...
- **Start an optional local worker.** `astra_agent_start` runs a bounded model/tool loop against a running loopback LM Studio API, using an explicit model and tool whitelist. Inspect it with `astra_agent_status` and cancel with `astra_agent_cancel`. [Setup and limits](docs/STUDIO.md#optional-local-model-workers).
...
| Capability | Astra 3 provides | What remains external or unimplemented |
| --- | --- | --- |
...
See [CONTRIBUTING.md](CONTRIBUTING.md), [security reporting](SECURITY.md) and the issue templates. Astra is [MIT licensed](LICENSE); dependencies and model weights retain their own licenses. Release archives contain no personal profiles, notes, model weights or account configuration.

## Graphical setup and community addons

Both platform archives include a native setup window with **Install / upgrade Astra**, **Install addon…** and **My addons**. See [setup instructions](docs/SETUP.md) for prerequisites and the extracted-folder addon workflow. Browse or submit community addons in the [addon area](addons/README.md); publication is opt-in.
