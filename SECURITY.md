...
Astra runs local tools with the permissions of the account that launches it. Its private desktop separates pointer, keyboard, display, and application sessions for simultaneous work. It is **not a security sandbox**: shell commands and filesystem tools can access or change data available to that account, and browser sessions can perform actions in accounts signed into Astra.
...
- Run Astra as an ordinary user. Its stdio integration is intended for a trusted local MCP host. The optional HTTP control panel has the local-only boundaries described below; do not forward either interface to a public network.
...
- Review consequential actions in the MCP host when practical. Astra does not provide an independent approval system.
...
Windows use is through a Linux environment under WSL2; the isolation described here applies to Astra's Linux desktop. It does not grant control of native Windows application windows or isolation from all host files accessible to WSL.
...
The last known link is stored in `control.json` under Astra's private data directory with owner-only file permissions. The descriptor remains after shutdown and is replaced on the next start; its presence does not prove a live session. `npm run control` checks liveness before printing it. Closing the viewer does not stop Astra. Pause gates new controlled calls; Stop requests cancellation of active calls, jobs and workers. Neither rolls back completed changes nor guarantees immediate termination of a previously dispatched action.

Starting `astra_agent_start` delegates the explicitly allowed tools for that bounded run. Its internal tool calls do **not** request a fresh approval from the MCP host on each dispatch. Authorizing a shell, job or browser tool gives the worker that tool's broad capabilities; a name whitelist is not filesystem or network confinement. Workers share Astra's files and desktop.

Worker model requests go only to configured loopback HTTP(S) `/v1` endpoints, with no API keys or inherited authorization headers; redirects and remote endpoint addresses are rejected. This constrains Astra's inference endpoint, not what an allowed tool can access or what a local server might do internally. Trust the local model service and its operator. Cancellation can leave an internal tool in flight; check status and actual state before another attempt.
