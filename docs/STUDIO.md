...
Linux filesystem tools include your real home directory by default. `ASTRA_FILE_ROOTS` adds up to 16 absolute existing directories; paths are resolved against real filesystem locations, including symlinks. This is a file-tool boundary, not a restriction on everything a launched process can access.
...
On 2026-09-09, Blender **4.5.13 LTS** was downloaded from Blender's official release server, checked against its published SHA-256, and exercised through Astra on **Linux Mint 22.3 x64 / Ubuntu 24.04 base**:
