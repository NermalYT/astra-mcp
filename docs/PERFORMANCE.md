...
Measured on 2026-09-09 using Node 22.23.2 on Linux Mint 22.3 x64 (Ubuntu 24.04 base). The baseline is Astra **v2.0.0**; the revised implementation is **v2.1.0**. These are local tool-call measurements, not a guarantee of model reasoning speed, production application load time, or rendering performance.
...
- The GUI opened on Astra's private display; the full viewport, menus, outliner and properties were visually verified after about 10 seconds. The initial launch response alone did not establish application readiness.
