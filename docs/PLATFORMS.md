...
| --- | --- | --- |
| `Astra-<version>-Linux-Ubuntu-LTS.tar.gz` | Read `START_HERE.md`; run `bash INSTALL_ASTRA.sh` in a terminal | A fresh source copy under the Linux user's `~/.local/share/astra-mcp/sources/` |
| `Astra-<version>-Windows-11-Pro-WSL2.zip` | Read `START_HERE.md`; run `INSTALL_ASTRA.cmd` | A fresh source copy inside the selected WSL2 distribution |

Both archives contain the same public Astra source and tools, with a platform-specific starter document and launcher at the top level. `README.md` remains the full project overview. Dependencies, model weights, browser profiles, application accounts, Blender, Unity, and GPU drivers are not bundled. Initial dependency installation needs internet access. The launcher remains open to show errors; it does not restart Windows or initialize your WSL user for you.
...
python3 scripts/package-release.py --verify dist/Astra-2.1.0-Linux-Ubuntu-LTS.tar.gz
```
...
Long commands use `astra_job_start`, `astra_job_status`, and `astra_job_cancel`. Jobs run with Astra's private application profile and display; project files are selected through an explicit working directory in the configured file roots. Browser sessions and native app profiles remain separate. Use a separate project copy when concurrent editing or application locks would cause conflicts. See [studio workflows](STUDIO.md) for argument examples, mounts, licenses, job lifetimes, and output checks.

The private Xvfb display is not a GPU-accelerated desktop guarantee. Blender background rendering can be useful without a GUI; GPU compute still depends on the actual hardware, driver, build, and selected device. Unity's documented Linux Editor requirements do not certify XFCE/Xvfb, WSL2, or Linux ARM64 merely because Astra runs there. The [studio guide](STUDIO.md#graphics-and-validation) links the vendor requirements and distinguishes tested runtime behavior from untested application workflows.
