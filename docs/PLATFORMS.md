...
On Linux, update your checkout and rerun setup. On Windows, each setup stages a fresh source directory under `~/.local/share/astra-mcp/sources/` in WSL. Old source directories remain available for configuration rollback; remove obsolete ones manually only after you no longer need those backups. Browser profiles and data reside under `~/.local/share/astra-mcp` independently of the source version. Back up that directory if you need to preserve browser sessions.
