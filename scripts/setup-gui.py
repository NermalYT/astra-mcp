...
def node_command():
    candidates = [shutil.which('node'), str(Path.home() / '.local/share/astra-mcp/node/bin/node')]
    for candidate in candidates:
        if candidate and Path(candidate).is_file():
            ...
            if result.returncode == 0:
                return candidate
    raise RuntimeError('Install Astra first: Node.js 22 or newer is required for addons.')
...
def addon_command(action, folder=None):
    ...
    if action != 'list':
        ...
        if not target.is_dir():
            raise ValueError('Choose the extracted folder containing astra-addon.json.')
...
def install_worker(result_path):
    ...
    try:
        with result.with_suffix('.log').open('wb') as output:
            child = subprocess.Popen(['bash', str(ROOT / 'platform/Linux/INSTALL_ASTRA.sh'), '--no-gui'], env={**os.environ, 'ASTRA_NO_PAUSE': '1'}, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            while True:
...
def main():
    ...
    try:
        ...
        print('Graphical setup needs python3-gi and gir1.2-gtk-3.0. Install those packages or run bash platform/Linux/INSTALL_ASTRA.sh --no-gui.', file=sys.stderr)
        return 1
    if args.check:
        return 0
    if os.geteuid() == 0:
        print('Launch Astra setup as your ordinary desktop user, not root.', file=sys.stderr)
        return 1
    css = Gtk.CssProvider()
    ...
    window = Gtk.Window(title='Astra · Setup & addons')
    ...
    label('Install a fresh source copy and connect LM Studio. Existing configuration is backed up; six legacy Astra connections are replaced. A terminal handles sudo prompts.')
    buttons = Gtk.Box(spacing=10)
    layout.pack_start(buttons, False, False, 0)
    install = Gtk.Button(label='Install / upgrade Astra')
    ...
    label('Studio applications and models are installed separately. Addons run with your account permissions. Restart Astra after installing an addon.', 'subtitle')
    state = {'busy': False}
    ...
    def on_choose(_):
        ...
        def inspected(details):
            ...
            prompt.format_secondary_text(details[:6000] + '\n\nIt can execute code with your account permissions when Astra restarts. Install only source you trust. Nothing is uploaded to GitHub.')
            trusted = prompt.run() == Gtk.ResponseType.OK
            prompt.destroy()
            if trusted:
                background(lambda: call_addon('install', selected), lambda value: show('Addon installed · restart Astra to activate', value))
        background(lambda: call_addon('inspect', selected), inspected)
    def on_install(_):
        terminal = shutil.which('x-terminal-emulator')
        if not terminal:
            show('No desktop terminal found', 'Run bash platform/Linux/INSTALL_ASTRA.sh --no-gui from a terminal.')
            return
        directory = tempfile.TemporaryDirectory(prefix='astra-setup-')
        ...
        def poll():
            ...
            try:
                ...
                show('Installed · restart LM Studio and enable mcp/astra' if code == 0 else 'Installation failed · review the terminal output', result.with_suffix('.log').read_text(errors='replace')[-30000:] if result.with_suffix('.log').exists() else f'Installer exit code: {code}. See docs/SETUP.md for recovery.')
            finally:
                busy(False)
                directory.cleanup()
            return False
