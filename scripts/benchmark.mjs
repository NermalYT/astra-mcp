...
const temporary=await fs.mkdtemp(path.join(os.tmpdir(),'astra-benchmark-'));
const rows=[];
const localXvfb=path.join(os.homedir(),'.local/share/astra-mcp/runtime/usr/bin/Xvfb');
...
try{
 for(let i=0;i<opts.trials;i++){
  const env={...process.env,ASTRA_DATA_DIR:path.join(temporary,String(i)),ASTRA_SERVER_MODULE:opts.server,ASTRA_BENCHMARK_CORE_ONLY:'1'};
  if(!env.ASTRA_XVFB)await fs.access(localXvfb).then(()=>{env.ASTRA_XVFB=localXvfb;}).catch(()=>{});
