#!/usr/bin/env python3
"""Build and verify a self-contained installer without installing Astra.

Usage: python3 scripts/build-installer.py [--output /path/install-astra.py]
...
package-lock.json
package.json
platform/Linux/INSTALL_ASTRA.sh
platform/Linux/README.md
platform/Windows/INSTALL_ASTRA.cmd
...
def load_payload():
    ...
    baseline = {"format": 1, "project": "astra-mcp", "version": payload["version"],
...
def main(argv=None):
    ...
    if options.extract_only:
        ...
        destination = Path.home() / ".local/share/astra-mcp/sources" / (version + "-" + stamp)
    extracted = extract_source(destination, decoded)
    print("Verified {} Astra {} source files in {}".format(len(decoded), payload["version"], extracted), flush=True)
    if options.extract_only:
        return 0
...
if __name__ == "__main__":
    try:
        ...
        print("Astra installer: " + str(error), file=sys.stderr)
        sys.exit(1)
'''
...
def build_source(files):
    ...
    compile(result, "install-astra.py", "exec")
    return result.encode(), digest


def verify_bootstrap(content, files):
    with tempfile.TemporaryDirectory(prefix="astra-bootstrap-check-") as temporary:
        root = Path(temporary)
        bootstrap = root / "install-astra.py"
        ...
        baseline = {"format": 1, "project": "astra-mcp", "version": version,
...
def main():
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--output", type=Path, default=Path.home() / "Downloads/install-astra.py")
