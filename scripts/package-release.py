...
python3 scripts/package-release.py --verify dist/Astra-VERSION-Linux-Ubuntu-LTS.tar.gz
...
""".split())
PLATFORMS = {
    "linux": ("Linux-Ubuntu-LTS", "platform/Linux/README.md", "platform/Linux/INSTALL_ASTRA.sh", ".tar.gz"),
    "windows": ("Windows-11-Pro-WSL2", "platform/Windows/README.md", "platform/Windows/INSTALL_ASTRA.cmd", ".zip"),
}
...
def public_source():
    spec = importlib.util.spec_from_file_location("astra_installer_builder", ROOT / "scripts/build-installer.py")
...
def release_files(source, platform, version):
    ...
    manifest = {"format": 1, "project": "astra-mcp", "version": version, "platform": platform,
...
def verify(content, kind, expected=None):
    ...
    if (manifest.get("format") != 1 or manifest.get("project") != "astra-mcp"
    ...
    if top != "Astra-" + str(manifest["version"]) + "-" + label or kind != expected_kind:
    ...
    with tempfile.TemporaryDirectory(prefix="astra-release-check-") as temporary:
...
def self_test():
    ...
    for platform, (label, _, _, kind) in PLATFORMS.items():
        ...
        packed = pack(files, "Astra-0.0.0-" + label, kind)
        assert pack(files, "Astra-0.0.0-" + label, kind) == packed
        assert verify(packed, kind, files)["verified"]
        changed = dict(files)
        changed["README.md"] = (b"tampered", 0o644)
        try:
            verify(pack(changed, "Astra-0.0.0-" + label, kind), kind)
        except ValueError:
            pass
        else:
            raise AssertionError("Tampering was not rejected.")
        try:
            read_archive(pack({"source": (b"x", 0o644), "source/file": (b"x", 0o644)}, "Astra-test", kind), kind)
        except ValueError:
            pass
        else:
