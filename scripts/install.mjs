...
function privateXvfb() {
  const candidate = process.env.ASTRA_XVFB || path.join(process.env.ASTRA_DATA_DIR || path.join(os.homedir(), '.local/share/astra-mcp'), 'runtime/usr/bin/Xvfb');
...
function entryForInstall() {
  return { command: process.execPath, args: [path.join(project, 'src', 'server.mjs')],
    env: { ASTRA_DATA_DIR: path.resolve(process.env.ASTRA_DATA_DIR || path.join(os.homedir(), '.local', 'share', 'astra-mcp')),
      ...(privateXvfb() ? { ASTRA_XVFB: privateXvfb() } : {}),
...
export async function verifyServer(entry) {
  ...
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-install-check-'));
  const client = new Client({ name: 'astra-installer-check', version: VERSION });
  const transport = new StdioClientTransport({ command: entry.command, args: entry.args,
    env: { ...process.env, ...entry.env, ASTRA_DATA_DIR: temporary }, stderr: 'pipe' });
  let log = '';
  ...
  try {
    ...
    if (!listed.tools?.some(tool => tool.name === 'astra_status')) throw new Error('astra_status was not advertised.');
    const result = await client.callTool({ name: 'astra_status', arguments: {} }, undefined, { timeout: 15000 });
    if (result.isError) throw new Error('astra_status returned an error.');
...
export async function main(args = process.argv.slice(2)) {
  ...
  if (process.getuid?.() === 0) throw new Error('Install Astra as your ordinary Linux/WSL user, not root. The installer uses sudo only for system packages.');
  ...
  if (!options.skipDeps) {
    ...
    try {
      ...
      await page.setContent('<title>Astra install check</title><p>Background browser ready</p>');
      if (await page.title() !== 'Astra install check') throw new Error('Background browser verification failed.');
  ...
  await fs.mkdir(entry.env.ASTRA_DATA_DIR, { recursive: true, mode: 0o700 });
  if (!options.skipConfig) {
    const result = await writeLmStudioConfig({ configPath: options.configPath, entry, replaceLegacy: options.replaceLegacy });
    console.log(`Registered Astra in ${result.configPath}`);
    if (result.backup) console.log(`Previous configuration: ${result.backup}`);
  }
  console.log('Astra installation complete. Restart LM Studio, select a tool-capable model, and enable mcp/astra.');
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch(error => { console.error(`Astra install failed: ${error.message}`); process.exitCode = 1; });
}
