...
function Invoke-AstraAddon {
    param([string]$Action, [string]$Folder)
    if ($Action -notin @('list', 'inspect', 'install')) { throw 'Unsupported addon action.' }
    $entry = (Get-Content -LiteralPath (Join-Path $env:USERPROFILE '.lmstudio\mcp.json') -Raw | ConvertFrom-Json).mcpServers.astra
    if ($entry.command -ne 'wsl.exe') { throw 'Install Astra for Windows first. Expected the WSL2 Astra entry in LM Studio configuration.' }
    $launch = @($entry.args)
    if ($launch.Count -lt 5 -or $launch[-1] -notmatch '/src/server\.mjs$') { throw 'Unrecognized Astra launch configuration. Use the documented addon CLI for custom configurations.' }
    $distroIndex = [Array]::IndexOf($launch, '--distribution')
    if ($distroIndex -lt 0 -or $distroIndex + 1 -ge $launch.Count) { throw 'Missing WSL distribution in Astra configuration.' }
...
if ($Worker) {
    ...
    try {
        ...
        elseif ($Worker -eq 'addon') { Invoke-AstraAddon -Action install -Folder $AddonPath }
...
[xml]$markup = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="Astra / Setup and addons" Width="820" Height="700" MinWidth="720" MinHeight="620" WindowStartupLocation="CenterScreen" Background="#101820" Foreground="#E7F2F3">
...
<TextBlock Text="ASTRA / 3.0" Foreground="#96B9C0"/>
...
<WrapPanel Grid.Row="4"><Button Name="Install" Content="Install / upgrade Astra" Background="#117D70"/><Button Name="Addon" Content="Install addon..."/><Button Name="List" Content="My addons"/></WrapPanel>
...
$script:astraOperation = $null
function Start-AstraOperation {
    param([string]$Kind, [string]$Folder = '')
    $result = Join-Path ([IO.Path]::GetTempPath()) ('astra-setup-' + [Guid]::NewGuid().ToString('N') + '.json')
    ...
    $script:astraOperation = @{ process = $process; result = $result; kind = $Kind }
...
$install.Add_Click({ try { Start-AstraOperation -Kind install } catch { $status.Text = 'Could not start setup'; $details.Text = $_.ToString() } })
$addon.Add_Click({
    $picker = New-Object System.Windows.Forms.FolderBrowserDialog
    $picker.Description = 'Choose the extracted folder containing astra-addon.json'
    try {
        if ($picker.ShowDialog() -ne [Windows.Forms.DialogResult]::OK) { return }
        $folder = $picker.SelectedPath
        $inspection = (Invoke-AstraAddon -Action inspect -Folder $folder | Out-String)
        ...
        if ($answer -eq 'OK') { Start-AstraOperation -Kind addon -Folder $folder }
    } catch { $status.Text = 'Addon inspection failed'; $details.Text = $_.ToString() }
    finally { $picker.Dispose() }
})
$list.Add_Click({ try { $details.Text = (Invoke-AstraAddon -Action list | Out-String); $status.Text = 'Installed addons' } catch { $details.Text = $_.ToString(); $status.Text = 'Cannot list addons' } })
$timer = New-Object Windows.Threading.DispatcherTimer
$timer.Interval = [TimeSpan]::FromSeconds(1)
$timer.Add_Tick({
    if ($null -eq $script:astraOperation) { return }
    $op = $script:astraOperation
    if (Test-Path -LiteralPath $op.result) {
        try {
            $result = Get-Content -LiteralPath $op.result -Raw | ConvertFrom-Json
            $status.Text = if ($result.exit_code -eq 0) { 'Completed / restart LM Studio and enable mcp/astra' } else { 'Failed / read the error in the console' }
...
$window.Add_Closing({ param($sender, $eventArgs) if ($null -ne $script:astraOperation) { $eventArgs.Cancel = $true; $status.Text = 'Wait for the running action or close its console before exiting setup' } })
$timer.Start()
