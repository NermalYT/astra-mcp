...
try {
  ...
  let dataDir = process.env.ASTRA_DATA_DIR || path.join(os.homedir(), '.local/share/astra-mcp');
  const seen = new Set();
  while (args.length) {
...
} catch (error) { console.error(`Astra maintenance failed: ${error.message}`); process.exitCode = 1; }
