...
test('config merge preserves unrelated integrations and removes legacy only when requested', () => {
  ...
  const entry = { command: '/path with spaces/node', args: ['/path/astra/server.mjs'] };
  assert.equal(Object.keys(mergeConfig(input, entry).mcpServers).length, 4);
  const merged = mergeConfig(input, entry, true);
  assert.deepEqual(Object.keys(merged.mcpServers), ['custom', 'astra']);
  assert.deepEqual(merged.mcpServers.astra, entry);
...
test('installer writes atomically with exact backup and never destroys malformed JSON', async () => {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-config-test-'));
  try {
    ...
    assert.equal(JSON.parse(await fs.readFile(configPath, 'utf8')).mcpServers.astra.args[0], '/tmp/updated.mjs');
