...
const manifest = JSON.parse(await fs.readFile(path.join(example, 'astra-addon.json'), 'utf8'));
async function fixture(fn) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-addons-test-'));
  try {
...
test('manifest validates required compatibility, namespaces, platform and executable arguments', () => {
  ...
  for (const id of ['../hello', 'astra', 'astra_custom', 'Hello', 'hello-addon', 'x'.repeat(33), '__proto__']) assert.throws(() => validateAddonManifest({ ...manifest, id }), /id|namespace/);
  for (const fields of [{ astra_compat: '*' }, { astra_compat: '>=2.0.0' }, { platforms: ['windows'] }, { platforms: ['linux', 'linux'] }, { version: 'latest' }, { version: '01.0.0' }, { surprise: true }, { server: { command: '{node}', args: 'server.mjs' } }, { server: { ...manifest.server, env: { X: 4 } } }]) assert.throws(() => validateAddonManifest({ ...manifest, ...fields }));
  assert.equal(addonTools.length, 1);
  assert.equal(addonTools[0].name, 'astra_addons');
});
...
test('inspect and installation never execute code; source is independently copied and persists privately', async () => fixture(async ({ source, dataDir, manager }) => {
  ...
  await fs.writeFile(path.join(source, 'astra-addon.json'), JSON.stringify(custom));
...
test('enable/disable/remove are persistent, require trust for execution, and keep recoverable files', async () => fixture(async ({ source, manager }) => {
  ...
  assert.equal(JSON.parse(await fs.readFile(path.join(removed.recovery_path, 'astra-addon.json'), 'utf8')).id, 'hello');
...
test('tool wrapper uses actual schema and invalid/cancelled actions cannot mutate', async () => fixture(async ({ source, dataDir, manager }) => {
  const result = JSON.parse((await callAddonTool('astra_addons', { action: 'inspect', path: source }, manager)).content[0].text);
  assert.equal(result.manifest.id, 'hello');
  await assert.rejects(callAddonTool('astra_addons', { action: 'install', path: source }, manager), /trust:true/);
  const controller = new AbortController(); controller.abort();
  await assert.rejects(callAddonTool('astra_addons', { action: 'install', path: source, trust: true }, manager, controller.signal), /cancelled/);
  await missing(dataDir);
  await assert.rejects(manager.disable('../escape'), /id/);
  await assert.rejects(manager.remove('astra'), /namespace/);
  await assert.rejects(callAddonTool('other', {}, manager), /Unknown addon tool/);
  await assert.rejects(callAddonTool('astra_addons', { action: 'launch' }, manager), /Unknown addon action/);
}));
...
test('incompatible, altered and invalid installed manifests never become backend configurations', async () => fixture(async ({ source, dataDir, manager }) => {
  ...
  await fs.writeFile(path.join(source, 'astra-addon.json'), JSON.stringify(winOnly));
  ...
  await fs.writeFile(path.join(source, 'astra-addon.json'), JSON.stringify(manifest));
  const installed = await manager.install(source, { trust: true });
  await fs.writeFile(path.join(installed.path, 'astra-addon.json'), JSON.stringify({ ...manifest, id: 'other' }));
  assert.deepEqual(await manager.backendConfig(), {});
  assert.match((await manager.list()).addons[0].error, /differs/);
  await fs.writeFile(path.join(installed.path, 'astra-addon.json'), '{');
  assert.deepEqual(await manager.backendConfig(), {});
...
test('example speaks actual MCP stdio initialize, tools/list and tools/call', async () => {
  const client = new Client({ name: 'astra-addon-test', version: '1.0.0' });
  ...
  try {
    ...
    const result = await client.callTool({ name: 'greet', arguments: { name: 'Astra' } });
    assert.match(result.content[0].text, /Hello, Astra! Your private Astra addon is working/);
...
test('CLI supports local inspect/install/list/disable/remove and rejects extra flags', async () => fixture(async ({ source, dataDir }) => {
  const run = args => exec(process.execPath, [path.join(repo, 'scripts', 'addons.mjs'), ...args], { env: { ...process.env, ASTRA_DATA_DIR: dataDir } });
...
test('vendor studio templates inspect and register without starting proprietary applications', async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-studio-manifests-'));
  try {
