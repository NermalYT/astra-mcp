...
async function fixture(t, options = {}) {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-studio-test-'));
  const appHome = path.join(home, 'private'); await fs.mkdir(appHome);
  const env = { ...process.env, ASTRA_APP_HOME: appHome, ASTRA_FILE_ROOTS: '[]', DISPLAY: ':987', XAUTHORITY: '/private/auth', DBUS_SESSION_BUS_ADDRESS: 'private-bus' };
...
test('job schemas validate defaults and reject malformed calls without dispatch', async t => {
  ...
  const start = inputValidator(studioTools.find(tool => tool.name === 'astra_job_start').inputSchema);
...
test('actual jobs retain argv, private runtime environment and split Unicode output', { skip: !linux }, async t => {
  ...
  assert.equal(JSON.parse((await callStudioTool('astra_job_status', { job_id: started.job_id }, manager)).content[0].text).job_id, started.job_id);
});
...
test('timeout terminates a child that ignores SIGTERM and reports timed_out', { skip: !linux }, async t => {
  ...
  assert.equal((await callStudioTool('astra_job_status', { job_id: job.job_id }, manager)).isError, true);
...
test('bounded output/history, nonzero exit and missing executable are explicit', { skip: !linux }, async t => {
  ...
  const missing = await manager.start({ command: 'astra-nonexistent-test-executable-1234', cwd: home });
...
test('abrupt Astra parent death cleans job and descendant processes', { skip: !linux }, async t => {
...
test('supervisor startup failure reports failed and an MCP error, not cancellation', { skip: !linux }, async t => {
  ...
  const result = await callStudioTool('astra_job_start', { command: process.execPath, cwd: home }, manager);
