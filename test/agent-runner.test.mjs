...
test('runner is idle by default; explicit start sends only allowed schemas without environment credentials', async t => {
  ...
  const start = await callAgentTool('astra_agent_start', options(), runner);
  ...
  const status = await callAgentTool('astra_agent_status', { agent_id: id, cursor: 0, max_chars: 12 }, runner);
...
test('agent/control/discovery wrappers cannot enter an agent whitelist', async t => {
  const names = ['astra_agent_start', 'astra_agent_status', 'astra_agent_cancel', 'astra_control_panel', 'astra_control_stop', 'astra_call', 'astra_tools', 'astra_schedule_start'];
...
test('cancellation aborts network work and close prevents new agents', async t => {
  ...
  await callAgentTool('astra_agent_cancel', { agent_id: run.agent_id }, runner);
...
test('turn limit is explicit and pre-cancelled starts dispatch nothing', async t => {
  ...
  assert.deepEqual(agentTools.map(x => x.name), ['astra_agent_start', 'astra_agent_status', 'astra_agent_cancel']);
});
