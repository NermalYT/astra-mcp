...
const packageFiles=version=>({'package.json':JSON.stringify({name:'astra-fixture',version,type:'module',dependencies:{}}),'package-lock.json':JSON.stringify({name:'astra-fixture',version,lockfileVersion:3,packages:{'':{name:'astra-fixture',version,dependencies:{}}}})});
async function fixture(t){
 const temporary=await fs.mkdtemp(path.join(os.tmpdir(),'astra-maintenance-test-')), root=path.join(temporary,'source'),dataDir=path.join(temporary,'data');
 const files={...packageFiles('3.0.0'),'README.md':'original readme\n','LICENSE':'fixture','src/server.mjs':'export const fixture = 1;\n','src/custom.mjs':'export const custom = 1;\n','scripts/install.sh':'#!/bin/sh\n','scripts/install.ps1':'# fixture\n','scripts/package-release.py':await fs.readFile(path.join(repo,'scripts/package-release.py'),'utf8'),'platform/Linux/README.md':'Linux\n','platform/Linux/INSTALL_ASTRA.sh':'#!/bin/sh\n','platform/Windows/README.md':'Windows\n','platform/Windows/INSTALL_ASTRA.cmd':'@echo off\n','addons/README.md':'Public addon index\n'};
 ...
 const baseline={format:1,project:'astra-mcp',version:'3.0.0',files:Object.entries(files).map(([name,content])=>({path:name,size:Buffer.byteLength(content),sha256:hash(content),mode:0o644}))};
...
function releaseFetch(bytes,requests=[]){return async(url,options)=>{requests.push({url,options});return url.endsWith('/SHA256SUMS')?new Response(hash(bytes)+'  Astra-3.0.1-Linux-Ubuntu-LTS.tar.gz\n'):new Response(bytes);};}
...
test('check parses source without executing it and reports syntax/package failures',async t=>{
 ...
 const result=await callServiceTool('astra_service',{action:'check'},f.manager);assert(result.isError);
...
test('official prepare verifies the real archive format and apply preserves unchanged-upstream customization',{skip:process.platform!=='linux'},async t=>{
 ...
 assert.equal(requests.length,2);assert(requests.every(r=>r.url.startsWith('https://github.com/NermalYT/astra-mcp/releases/download/v3.0.1/')&&r.options.redirect==='manual'&&!r.options.headers.Authorization));
...
test('upstream/local conflicts, dependency changes and tampered stages never overwrite current files',{skip:process.platform!=='linux'},async t=>{
 ...
 bytes=await archive(f,{'package.json':JSON.stringify({name:'astra-fixture',version:'3.0.1',type:'module',dependencies:{newPackage:'1.0.0'}})});
...
test('downloads refuse checksum mismatch, remote redirects and absent trusted baselines',async t=>{
 const f=await fixture(t);let manager=f.make({fetchImpl:async url=>new Response(url.endsWith('SHA256SUMS')?'0'.repeat(64)+'  Astra-3.0.1-Linux-Ubuntu-LTS.tar.gz\n':'bad')});
...
test('Windows release upgrades retain the Windows starter and ZIP selection', {skip:process.platform!=='linux'},async t=>{
 ...
 const bytes=await archive(f,{},'windows'),requests=[];const manager=f.make({fetchImpl:async(url)=>{requests.push(url);return new Response(url.endsWith('SHA256SUMS')?hash(bytes)+'  Astra-3.0.1-Windows-11-Pro-WSL2.zip\n':bytes)}});
