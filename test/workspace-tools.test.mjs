...
async function fixture(t, overrides = {}) {
  const home = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'astra workspace ')));
  ...
  const config = { dataDir: path.join(home, 'data'), home, env: { ...process.env, ASTRA_FILE_ROOTS: '[]', ASTRA_APP_HOME: appHome }, ...overrides };
...
test('writer revision lock works across independent Node processes', async t => {
  ...
  const childConfig = { dataDir: config.dataDir, home: config.home, env: { ASTRA_FILE_ROOTS: '[]' } };
...
test('notes enforce encoded text limits, pagination, private permissions and safe schemas', async t => {
  ...
  const result = await callWorkspaceTool('astra_workspace_notes', { action: 'list', project }, manager);
...
test('version probes preserve argv/private HOME and explicitly report failure or deadline', async t => {
  ...
  const failure = createWorkspaceManager({ home, dataDir: path.join(home, 'fail-data'), env: { ASTRA_FILE_ROOTS: '[]' }, runVersion: async () => { throw { killed: true, message: 'timeout' }; } });
  assert.equal((await failure.probe({ executable: process.execPath, app: 'node' })).applications[0].version_probe, 'timed_out');
  const unhealthy = createWorkspaceManager({ home, dataDir: path.join(home, 'unhealthy'), env: { ASTRA_FILE_ROOTS: '[]' }, runtimeStatus: () => ({ healthy: false }), runVersion: () => assert.fail('must not dispatch') });
...
test('pre-cancelled workspace operations create no storage and dispatch no probes', async t => {
  ...
  await assert.rejects(callWorkspaceTool('astra_workspace_notes', { action: 'write', project, key: 'stopped', content: 'must not save' }, manager, controller.signal), /cancelled/);
  await assert.rejects(callWorkspaceTool('astra_app_probe', { apps: ['node'] }, manager, controller.signal), /cancelled/);
