...
async function fixture(t) {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-v3-lifecycle-'));
  ...
  const model = http.createServer(async (req, res) => {
    try {
      ...
      res.end(JSON.stringify({ choices: [{ finish_reason: hasToolResult ? 'stop' : 'tool_calls', message: hasToolResult
        ...
        : { role: 'assistant', content: null, tool_calls: [{ id: 'fixture_git', type: 'function', function: { name: 'astra_git_status', arguments: JSON.stringify({ root: temporary }) } }] }
      }] }));
  ...
  const client = new Client({ name: 'astra-v3-lifecycle', version: '1.0.0' });
  const transport = new StdioClientTransport({ command: process.execPath, args: [path.join(root, 'src/server.mjs')], stderr: 'pipe',
    env: { ...process.env, PATH: `${bin}${path.delimiter}${process.env.PATH || ''}`, ASTRA_DATA_DIR: dataDir,
      ASTRA_SETTINGS: settings, ASTRA_BACKENDS: backends, ASTRA_SKIP_RUNTIME: '1', ASTRA_TOOL_MODE: 'compact', ASTRA_CONTROL: '1',
      ASTRA_FILE_ROOTS: JSON.stringify([temporary]), ASTRA_APP_HOME: appHome } });
  ...
  const call = async (name, args = {}) => {
    const result = await client.callTool({ name: 'astra_call', arguments: { tool: name, arguments: args } }, undefined, { timeout: 15000 });
    assert(!result.isError, `${name}: ${result.content?.[0]?.text}\n${diagnostics}`); return value(result);
  };
  const panel = new URL((await call('astra_control_panel')).url);
  ...
  const startWork = async () => {
    const job = await call('astra_job_start', { command: process.execPath, args: [worker, markers.job, markers.child], cwd: temporary, timeout_ms: 20000 });
    ...
    const agent = model => call('astra_agent_start', { model, objective: 'Lifecycle fixture only.', allowed_tools: ['astra_git_status'], timeout_ms: 20000 });
    ...
    await until(async () => (await api('status')).agents.agents.some(run => run.agent_id === working.agent_id && run.in_flight_tool === 'astra_git_status'), 'agent tool dispatch');
...
test('panel Stop cancels real studio descendants, a pending model request and an agent tool without replay', { skip: !linux, timeout: 20000 }, async t => {
  ...
  const rejected = await f.client.callTool({ name: 'astra_call', arguments: { tool: 'astra_git_status', arguments: { root: f.dataDir } } });
