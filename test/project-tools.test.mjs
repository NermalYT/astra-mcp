...
async function fixture(t) {
  const home = await fsp.mkdtemp(path.join(os.tmpdir(), 'astra-project-test-'));
...
test('project read returns byte-exact hash, preserves CRLF/Unicode and bounds line output', async t => {
  ...
  const read = data(await call('astra_project_read', { path: target, start_line: 2, line_count: 1 }));
  ...
  const clipped = data(await call('astra_project_read', { path: target, max_chars: 100 }));
  ...
  const empty = data(await call('astra_project_read', { path: target })); assert.equal(empty.total_lines, 0); assert.equal(empty.sha256, hash(''));
});
...
test('literal edits require a fresh hash and a unique non-regex match', async t => {
  ...
  const read = data(await call('astra_project_read', { path: target }));
  const edit = data(await call('astra_project_edit', { path: target, expected_sha256: read.sha256, old_text: 'a.$(literal)', new_text: '$&\\literal\n世界' }));
  ...
  failure(await call('astra_project_edit', { path: target, expected_sha256: read.sha256, old_text: 'const', new_text: 'let' }), /Conflict/);
  await fsp.writeFile(target, 'aaa');
  failure(await call('astra_project_edit', { path: target, expected_sha256: hash('aaa'), old_text: 'aa', new_text: 'z' }), /more than once/);
  failure(await call('astra_project_edit', { path: target, expected_sha256: hash('aaa'), old_text: 'missing', new_text: 'z' }), /not found/);
  assert.equal(await fsp.readFile(target, 'utf8'), 'aaa');
  assert(!(await fsp.readdir(root)).some(name => name.startsWith('.astra-edit-')));
});
...
test('exclusive create never overwrites a preexisting file and requires existing parents', async t => {
  ...
  const created = data(await call('astra_project_edit', { path: target, expected_sha256: 'absent', new_text: 'created' }));
  assert.equal(created.created, true); assert.equal(created.sha256, hash('created'));
  failure(await call('astra_project_edit', { path: target, expected_sha256: 'absent', new_text: 'overwritten' }), /already exists/);
  failure(await call('astra_project_edit', { path: path.join(root, 'missing-parent', 'new'), expected_sha256: 'absent', new_text: 'x' }), /ENOENT/);
  failure(await call('astra_project_edit', { path: path.join(root, 'bad'), expected_sha256: 'absent', old_text: 'x', new_text: 'x' }), /no old_text/);
...
test('last-moment content and creation conflicts preserve the human edit and clean temporary files', async t => {
  ...
  try { failure(await call('astra_project_edit', { path: target, expected_sha256: hash('original'), old_text: 'original', new_text: 'AI edit' }), /Conflict/); }
  ...
  try { failure(await call('astra_project_edit', { path: other, expected_sha256: 'absent', new_text: 'AI create' }), /concurrently/); }
  finally { fs.linkSync = realLink; }
  assert.equal(await fsp.readFile(other, 'utf8'), 'human create');
  assert(!(await fsp.readdir(root)).some(name => name.startsWith('.astra-edit-')));
});

test('two simultaneous Astra edits with one expected hash allow exactly one mutation', async t => {
  const { root, call } = await fixture(t); const target = path.join(root, 'parallel.txt');
  await fsp.writeFile(target, 'original');
  const outputs = await Promise.all(['first', 'second'].map(new_text => call('astra_project_edit', { path: target, expected_sha256: hash('original'), old_text: 'original', new_text })));
...
test('path boundaries, binary/encoding/size limits, and cancellation prevent mutations', async t => {
  ...
  failure(await call('astra_project_read', { path: path.dirname(home) }), /Access denied/);
  const target = path.join(root, 'data');
  await fsp.writeFile(target, Buffer.from([0, 1, 2])); failure(await call('astra_project_read', { path: target }), /Binary/);
  await fsp.writeFile(target, Buffer.from([0xff, 0xff])); failure(await call('astra_project_read', { path: target }), /UTF-8/);
  const handle = await fsp.open(target, 'w'); await handle.truncate(3 * 1024 * 1024); await handle.close();
  failure(await call('astra_project_read', { path: target }), /2 MiB/);
  const abort = new AbortController(); abort.abort();
  failure(await call('astra_project_edit', { path: path.join(root, 'cancelled'), expected_sha256: 'absent', new_text: 'x' }, { signal: abort.signal }), /cancelled/);
...
test('symlink leaf and directory paths are refused and FIFO reads never block', async t => {
  ...
  failure(await call('astra_project_read', { path: link }), /Symbolic-link/);
  failure(await call('astra_project_edit', { path: link, expected_sha256: hash('content'), old_text: 'content', new_text: 'changed' }), /Symbolic-link/);
  ...
  failure(await call('astra_project_edit', { path: path.join(directoryLink, 'new'), expected_sha256: 'absent', new_text: 'x' }), /Symbolic-link/);
  if (process.platform === 'linux') {
    const fifo = path.join(root, 'pipe'); execFileSync('mkfifo', [fifo]);
    failure(await call('astra_project_read', { path: fifo }), /regular file/);
  }
...
test('portable literal search handles globs, Unicode and default exclusions without rg', async t => {
  ...
  const result = data(await call('astra_project_search', { root, query: 'needle', glob: '**/*.js' }, { env: { ...process.env, PATH: root, Path: root } }));
  ...
  failure(await call('astra_project_search', { root, query: '.*', regex: true }, { env: { ...process.env, PATH: root, Path: root } }), /requires ripgrep/);
  failure(await call('astra_project_search', { root, query: 'x', glob: '../*' }), /relative paths/);
});
...
test('ripgrep search uses explicit literal/regex modes and ignores unsafe config', { skip: !rgPresent }, async t => {
  ...
  const result = data(await call('astra_project_search', { root, query: 'needle', glob: '**/*.js' }, { env: { ...process.env, RIPGREP_CONFIG_PATH: config } }));
  assert.equal(result.engine, 'ripgrep'); assert.equal(result.matches.length, 1); assert.equal(result.matches[0].column, 4);
  const literal = data(await call('astra_project_search', { root, query: 'literal $(x).*' })); assert.equal(literal.matches.length, 1);
  const regex = data(await call('astra_project_search', { root, query: 'needl[e]', regex: true, glob: '*.txt' })); assert.equal(regex.matches[0].path, 'other.txt');
  failure(await call('astra_project_search', { root, query: '(', regex: true }), /ripgrep failed/);
});
...
test('search result and escaped-text bounds are reported explicitly', async t => {
  ...
  for (const env of [process.env, { ...process.env, PATH: root, Path: root }]) {
    const result = data(await call('astra_project_search', { root, query: 'match', max_results: 2, max_chars: 1000 }, { env }));
...
test('Git status/diffs are read-only, bounded and suppress configured external programs', { skip: !gitPresent }, async t => {
  ...
  git(['-c', 'user.name=Astra Test', '-c', 'user.email=astra-test@example.invalid', 'commit', '-m', 'fixture']);
  ...
  const result = data(await call('astra_git_status', { root, diff: true, max_chars: 40000 }, { env: { ...process.env, GIT_DIR: '/nonexistent-should-not-be-used' } }));
  ...
  const bounded = data(await call('astra_git_status', { root, diff: true, max_chars: 100 }));
  ...
  const outsideRepo = path.dirname(root); failure(await call('astra_git_status', { root: outsideRepo }), /not a git repository/);
});
...
test('Git cancellation cleans descendant processes and cannot hang on inherited pipes', { skip: process.platform !== 'linux' }, async t => {
  ...
  const pending = call('astra_git_status', { root }, { signal: abort.signal, env: { ...process.env, PATH: bin + path.delimiter + process.env.PATH } });
