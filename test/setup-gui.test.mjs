...
with tempfile.TemporaryDirectory(prefix='astra setup ') as tmp:
    os.environ['ASTRA_DATA_DIR'] = str(pathlib.Path(tmp)/'private')
...
spec = importlib.util.spec_from_file_location('setup', root/'scripts/setup-gui.py')
setup = importlib.util.module_from_spec(spec); spec.loader.exec_module(setup)
with tempfile.TemporaryDirectory(prefix='astra worker ') as tmp:
    setup.ROOT = pathlib.Path(tmp)
    launcher = setup.ROOT/'platform/Linux/INSTALL_ASTRA.sh'
