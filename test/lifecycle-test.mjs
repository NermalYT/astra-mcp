...
const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'astra-lifecycle-'));
...
async function testRuntimeParentDeath() {
  ...
  try {
    ...
    if (directory) {
      assert(path.dirname(directory) === os.tmpdir() && path.basename(directory).startsWith('astra-display-'));
...
const httpServer = http.createServer((request, response) => {
  ...
  response.end(`<!doctype html><title>Astra lifecycle ${label}</title><h1>Independent ${label} session</h1>`);
});
...
async function testBrowserLeases() {
  ...
  const open = async label => {
    const client = new Client({ name: `astra-lifecycle-${label}`, version: '1.0.0' });
    const transport = new StdioClientTransport({ command: process.execPath, args: [path.join(base, 'start-browser.mjs')], env: { ...process.env, ASTRA_BROWSER_PROFILE: profileRoot }, stderr: 'pipe' });
