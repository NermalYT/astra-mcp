...
const temporary = await fs.mkdtemp(path.join(os.homedir(), '.astra-integration-'));
...
const client = new Client({ name: 'astra-integration', version: '2.0.0' });
...
const fixtureTitle = `Astra integration ${path.basename(temporary)}`;
const desktopPath = path.join(os.homedir(), '.local/share/applications', `astra-integration-${path.basename(temporary)}.desktop`);
...
const httpServer = http.createServer((request, response) => {
  ...
  response.end(`<!doctype html><html><head><title>Astra ${second ? 'second' : 'first'} fixture</title></head><body>
    <main><h1>${second ? 'Second tab content' : 'Astra browser fixture'}</h1>
...
int main(int argc, char **argv) {
  ...
  XClassHint hint = { "astra-integration", "AstraIntegration" }; XSetClassHint(d,w,&hint);
  ...
  while (1) {
    ...
    FILE *f=fopen(out,"w"); if(f) {
      ...
      const char *names[] = { "DISPLAY", "HOME", "ASTRA_APP_HOME", "XDG_RUNTIME_DIR", "XDG_CONFIG_HOME", "XDG_CACHE_HOME", "XDG_DATA_HOME", "XDG_STATE_HOME" };
...
try {
  ...
  const status = jsonOf(await call('astra_status'));
  assert(!Array.isArray(status), 'Requires Astra server v2 (refusing desktop actions under v1)');
  ...
  const envResult = await shell('python3 -c ' + shellQuote('import json, os; print(json.dumps({k: os.environ.get(k) for k in ["DISPLAY", "HOME", "ASTRA_APP_HOME", "XDG_RUNTIME_DIR", "XDG_CONFIG_HOME", "XDG_CACHE_HOME", "XDG_DATA_HOME", "XDG_STATE_HOME"]}))'));
  ...
  const appHome = path.join(process.env.ASTRA_DATA_DIR || path.join(os.homedir(), '.local/share/astra-mcp'), 'app-homes', `display-${status.runtime.display.slice(1)}`);
  assert.equal(privateEnv.ASTRA_APP_HOME, appHome);
  ...
  await call('filesystem_write_text', { path: file, content: 'Astra file roundtrip — Ω\n' });
  assert.equal(jsonOf(await call('filesystem_read_text', { path: file })).content, 'Astra file roundtrip — Ω\n');
  ...
  assert(fixtureWindow.wm_class.toLowerCase().includes('astraintegration'), 'wmctrl class must not be swapped with hostname');
  ...
  const unknown = await call('no_such_astra_tool', {}, { allowError: true }); assert(unknown.isError);
  ...
  if (process.env.ASTRA_BENCHMARK_CORE_ONLY !== '1') {
    const observed = jsonOf(await call('astra_desktop_state', { screenshot: false }));
    ...
    const batch = await call('astra_desktop_actions', { steps: [
    ...
    await waitFor(async () => {
      const state = await fs.readFile(statePath, 'utf8');
      return state.includes('CLICKS:2\n') && state.includes('KEYS:AstraNative42Batch\n');
    }, 'grouped native actions');
    const invalidBatch = await call('astra_desktop_actions', { steps: [
    ...
    const job = jsonOf(await call('astra_job_start', { command: process.execPath, cwd: temporary, args: ['-e',
    ...
    assert(job.job_id); await call('astra_desktop_state', { screenshot: false });
    const finished = await waitFor(async () => {
      const state = jsonOf(await call('astra_job_status', { job_id: job.job_id }));
    ...
    assert.equal((await call('astra_job_start', { command: 'astra-no-such-executable', cwd: temporary }, { allowError: true })).isError, true);
  ...
  // Astra includes automatic snapshots inline so navigation supplies usable refs.
