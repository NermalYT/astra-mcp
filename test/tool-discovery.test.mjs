...
const inventory = [
  ...
  ...discoveryTools, ...['astra_status', 'astra_help', 'astra_control_panel'].map(name => ({ name }))
];
test('compact advertising retains full schema discovery and excludes recursive wrappers', () => {
  ...
  assert(!searchTools(inventory, {}).names.includes('astra_call'));
...
test('portable prompt and guide fallbacks preserve user objective as content', () => {
  ...
  assert.throws(() => getPrompt('astra_studio', { objective: '' }), /objective/);
});
